...
document.getElementById('play1').innerHTML=sessionStorage.getItem("player1");
document.getElementById('play2').innerHTML=sessionStorage.getItem("player2");
...
var player_live1 = 0;
var player_live2 = 0;
var player_1up = false;
var player_2up = false;
let controls, count_anim = 0;
...
function addGround() {
	var woodTex = THREE.ImageUtils.loadTexture('../textures/images.jpg');
...
function addWalls() {
	for (var i = 0; i < 20; i++) {
		walls.push(new THREE.Mesh(new THREE.BoxGeometry(320, 320, 320), new THREE.MeshLambertMaterial({ map: THREE.ImageUtils.loadTexture('../textures/blocks.jpg') })));
		walls[i].position.x = -1125 + (i % 10) * 250;
		walls[i].position.z = -250 + (i % 4) * 250;
		walls[i].castShadow = true;
		walls[i].receiveShadow = true;
		walls[i].position.y = 50;
...
loader.load('scene.gltf', function (gltf) {
	...
	landscape.scale.set(45000, 45000, 45000);
	landscape.position.set(-18000, -19120, 0);
...
function gameOver(player) {
	...
	document.getElementById("go1").style.visibility = "visible";
	document.getElementById("go1").innerHTML = "PLAYER" + player + " WINS";
...
scene.background = new THREE.ImageUtils.loadTexture('../textures/smokeparticle.png');
...
var barrel1 = new THREE.Mesh(new THREE.CylinderGeometry(17, 18, 280, 150), new THREE.MeshPhongMaterial({ color: 0x5555aa }));
cannon1.rotation.x = -60 * Math.PI / 180;
barrel1.position.y = 40;
cannon1.add(barrel1);
cannon1.position.y = 5;
var turret1 = new THREE.Mesh(new THREE.SphereGeometry(100, 100, 30), new THREE.MeshPhongMaterial({ color: 0x3333aa }));
head1.add(turret1);
head1.add(cannon1);
head1.position.y = 25;
head1.position.z = 10;
...
var block1 = new THREE.Mesh(new THREE.BoxGeometry(250, 150, 250), new THREE.MeshPhongMaterial({ color: 0x2222aa }));
...
var barrel2 = new THREE.Mesh(new THREE.CylinderGeometry(17, 18, 280, 150), new THREE.MeshPhongMaterial({ color: 0xaa5555 }));
cannon2.rotation.x = -60 * Math.PI / 180;
barrel2.position.y = 40;
cannon2.add(barrel2);
cannon2.position.y = 5;
var turret2 = new THREE.Mesh(new THREE.SphereGeometry(100, 100, 30), new THREE.MeshPhongMaterial({ color: 0xaa3333 }));
head2.add(turret2);
head2.add(cannon2);
head2.position.y = 25;
head2.position.z = 10;
...
var block2 = new THREE.Mesh(new THREE.BoxGeometry(250, 150, 250), new THREE.MeshPhongMaterial({ color: 0xaa2222 }));
	...
	if (!player1Dead && !player2Dead) {
			...
			if (player1Shots[i].Box3.isIntersectionBox(player2.Box3)) {
				...
				if (player_live1 < 3) {
					...
					if (player_live1 == 2) {
						...
						document.getElementById("blue_img4").style.visibility = "visible";
						player_2up = true;
					}
					if (player_live1 == 3 && player_2up == true) {
		...
		for (i = 0; i < player2Shots.length; i++) {
			...
			if (player2Shots[i].Box3.isIntersectionBox(player1.Box3)) {
				...
				if (player_live2 < 3) {
					console.log("player2live!")
					if (player_live2 == 0) {
						player_2up = false;
						document.getElementById("blue_img1").style.visibility = "hidden";
					}
					if (player_live2 == 1) {

						document.getElementById("blue_img2").style.visibility = "hidden";
					}
					if (player_live2 == 2) {

						document.getElementById("blue_img3").style.visibility = "hidden";
						document.getElementById("img4").style.visibility = "visible";
						player_1up = true;
					}
					if (player_live2 == 3 && player_1up == true) {
						document.getElementById("blue_img4").style.visibility = "hidden";

					}
					player_live2++;
					console.log("player_2live" + player_live2);
					audioHitPlayer.play();
					audioHitPlayer.currentTime = 0;
				} else {
		...
		if (flag == false && p1fireRate == 60 && Key.isDown(Key.SHIFT)) {
			//create bullet
			player1Shots.push(new THREE.Mesh(new THREE.SphereGeometry(20, 45, 20), new THREE.MeshLambertMaterial({ color: 0xffff00 })));
		...
		if (flag == true && p2fireRate == 60 && Key.isDown(Key.FSLASH)) {
			//create bullet
			player2Shots.push(new THREE.Mesh(new THREE.SphereGeometry(20, 45, 20), new THREE.MeshLambertMaterial({ color: 0xffff00 })));
